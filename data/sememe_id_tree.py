...
def main(dataset):
    with open("./nature_form_label.json", "r", encoding="utf-8") as f:
    ...
    for label in all_label:
        if "B" in label:
            label_word = f"开始词{label.split('-')[-1]}"
        elif "I" in label:
            label_word = f"中间词{label.split('-')[-1]}"
        else:
            label_word ="其他词"
        words_list = []
        words_id_list = []
        label_token = list(jieba.cut(label_word, cut_all=True))
        print(label_token)
        align = []
        curr_wrd = 0  # start at 1, b/c of CLS
        buf = []
        buf_o = []
        for i in range(0, len(label_token)):  # ignore [SEP] final token
    ...
    with open(f"./{dataset}/label_sememes_tree.json", "w", encoding="utf-8") as f:
        f.write(json.dumps(label_id_list))

if __name__ == "__main__":
    startToken = "[CLS]"
    name = "CCKS2019"
    tokenizer = BertTokenizer.from_pretrained("../model_hub/chinese-roberta-wwm-ext")
    main(name)
