    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for step, batch_data in loop:
                ...
                loop.set_postfix(loss=loss.item(), mini_loss=mini_loss)
                global_step += 1
                # print(global_step)
                ...
                if grad_steps >= patience:
                    ...
                    return  # 退出训练函数
            print(
                f'Epoch [{epoch}/{self.epochs}] {global_step}/{self.total_step} loss:{loss_infos[global_step - 1]} mini_loss={mini_loss} steps={global_step}')
...
def main(data_name, model_path):
    ...
    torch.save(model.state_dict(), os.path.join(args.output_dir, f"{model_name}_{data_name}_best.bin"))
    model.to(device)
